...
-- Eternal is read-only. This audit enriches only non-Eternal source records.
-- New actions remain data-only until they have an authoritative collector ID
-- and an explicit RetreatUI HUD placement.
  ...
  if existing then
    ...
    for key, value in pairs(values) do
      if key ~= "aliases" and key ~= "auditRecord" and key ~= "hudApproved"
        and (existing[key] == nil or key == "auraID" or key == "buffID"
          or key == "runtimeID" or key == "runtimeIDs" or key == "learnedByAny") then
        existing[key] = value
      end
    end
    return existing
...
-- Fleshweaver actions: retained as audited data, never injected into Eternal.
AddAudit({
  name="Sanguine Mend", id=504085, learnedByAny={504085, 803880},
  runtimeIDs={504085, 803880}, category="rotation", hudRow="core", order=12,
  trackCooldown=true, sourceTab="Fleshweaver", source="Wago 25AZFWqQH",
})
AddAudit({
...
AddAudit({
  ...
  trackCooldown=true, targetDebuff=true, sourceTab="Fleshweaver", source="Wago 25AZFWqQH",
})
AddAudit({
...
AddAudit({
  ...
  trackDuration=true, auraTracker=true, sourceTab="Fleshweaver", source="Wago 25AZFWqQH",
})
...
AddAudit({
  ...
  glowWhenAuraID={706613}, sourceTab="Sanguine", source="Wago 25AZFWqQH",
})
AddAudit({
...
AddAudit({
  ...
  trackDuration=true, auraTracker=true, sourceTab="Sanguine", source="Wago 25AZFWqQH",
})
...
AddAudit({
  ...
  sourceTab="Accursed", source="Wago 25AZFWqQH",
})
AddAudit({
  name="Ravenous Strike", id=572551, category="rotation", hudRow="core", order=24,
  trackCooldown=true, sourceTab="Accursed", source="Wago 25AZFWqQH",
})

-- Active-only non-Eternal notifiers.
...
database.bloodmagePackAuditRevision = 3
database.bloodmagePackSource = "25AZFWqQH"
database.bloodmageEternalAuditPolicy = "read-only"
database.bloodmageAuditHUDPolicy = "data-only-until-collector-bound"
