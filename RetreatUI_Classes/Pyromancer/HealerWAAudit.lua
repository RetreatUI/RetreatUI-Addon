...
-- Only Pyromancer-specific spell IDs, replacement cooldown IDs and active
-- effects are retained. Audit records are data-only unless separately approved.
...
local function AddAudit(values)
  ...
  if record then
    ...
    for key, value in pairs(values) do
      if record[key] == nil or key == "runtimeID" or key == "runtimeIDs"
        or key == "auraID" or key == "buffID" or key == "learnedByAny" then
        record[key] = value
      end
    end
...
-- Replacement spell IDs used by Flameweaving/Pyrolancer builds.
...
-- Meteor remains excluded from the audited HUD expansion; only its verified
-- runtime correction is retained for the existing curated record.
Patch("Meteor", {
...
-- Source-audited actions are retained as data, not promoted to HUD rows.
AddAudit({
...
-- Active-only healer and skin states remain valid aura trackers.
...
database.pyromancerHealerWAAuditRevision = 2
database.pyromancerHealerWagoSource = "g1d1f6fiC"
database.pyromancerAuditHUDPolicy = "data-only"
