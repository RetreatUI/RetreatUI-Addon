...
-- This only reads the player's own Tinker pet. It does not restore party
-- utility or party interrupt tracking and it never renders question-mark icons.
...
local function EnsureFrames()
  ...
  for _, definition in ipairs(PET_ACTIONS) do
...
local function LayoutVisibleIcons(visible)
  local count = #visible
  for index, icon in ipairs(visible) do
    icon:ClearAllPoints()
    icon:SetPoint("TOP", container, "TOP", (index - (count + 1) / 2) * 26, 0)
  end
end
...
local function Update()
  ...
  for _, icon in ipairs(icons) do
    ...
    local _, _, texture = GetSpellInfo and GetSpellInfo(definition.id)
    if texture then
  ...
  if #visible > 0 then container:Show() else container:Hide() end
end
