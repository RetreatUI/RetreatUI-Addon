...
local function CreateInstallModuleRow(parent, key, definition, index, page)
  local theme = Theme()
  ...
  row.status:SetPoint("RIGHT", -76, 0)

  row.toggle = Button(row, "ON", 58, 22, function()
...
local function CreateInstallPage()
  ...
  local intro = Text(page, "Choose which profiles and integrations RetreatUI may apply.", 11, theme.muted)
  ...
  for index, key in ipairs(RUI.moduleOrder or {}) do
    ...
    page.rows[key] = CreateInstallModuleRow(page, key, definition, index, page)
  end
  ...
  page.Refresh = function()
    ...
    for _, key in ipairs(RUI.moduleOrder or {}) do
      ...
      if row.toggle then
        row.toggle:SetLabel(definition and definition.selectable == false and "REQUIRED" or (enabled and "ON" or "OFF"))
        row.toggle:SetEnabled(definition and definition.selectable ~= false)
      end

      if not enabled then
        SetStatus(row.status, "skipped", "DISABLED")
