...
-- beta.20 installer: the NaowhUI TBC installation flow and geometry, with the
-- TBC-only pages replaced by CoA equivalents. No component picker, no boss-mod
-- page and no class-specific macro page.
...
local function SetBorderColor(edges, color, alpha)
  for _, edge in ipairs(edges or {}) do edge:SetVertexColor(color[1], color[2], color[3], alpha or color[4] or 1) end
end

local function Button(parent, width, height, label)
  local b = CreateFrame("Button", nil, parent)
  b:SetSize(width, height)
  b.bg = Solid(b, "BACKGROUND", BG[1], BG[2], BG[3], .92); b.bg:SetAllPoints()
  b.border = Border(b, .25)
  b.text = b:CreateFontString(nil, "OVERLAY"); b.text:SetPoint("CENTER"); Font(b.text, 14); b.text:SetText(label or "")
...
local function Action(button, label, callback)
  button:Show(); button:SetLabel(label); button:SetEnabledState(true)
...
local function ResetPage()
  ...
  frame.subtitle:SetText(""); frame.desc1:SetText(""); frame.desc2:SetText(""); frame.desc3:SetText("")
  SetResult(nil, "")
end
...
local function RefreshSteps()
  ...
  frame.progress:SetMinMaxValues(0, #STEPS); frame.progress:SetValue(page)
...
render[2] = function()
  ...
  frame.desc1:SetText("Select the same layout resolution used by the original NaowhUI installer")
...
render[4] = function()
  ...
  frame.desc1:SetText("Select the TurboPlates layout matching the Naowh Plater resolution")
  frame.desc2:SetText("Only settings TurboPlates supports are translated; Plater mods/scripts are not recreated")
...
render[5] = function()
  ...
  Action(frame.option1, "Core", function()
    if not RUI.InstallNaowhGeneralWeakAuras then return false, "General WeakAura package is unavailable." end
...
render[6] = function()
  ...
  Action(frame.option1, "Import Class WA", function()
    if not RUI.InstallNaowhClassWeakAuras then return false, "CoA class WeakAura package is unavailable." end
    return RUI:InstallNaowhClassWeakAuras(className)
  end)
end
...
local function Build()
  ...
  frame:SetSize(FRAME_W, FRAME_H); frame:SetPoint("CENTER", UIParent, "CENTER", -(STEPS_W * .5), 0)
  ...
  local function Description(previous, y)
    local fs = frame:CreateFontString(nil, "OVERLAY"); Font(fs, 14); fs:SetWidth(FRAME_W - 40); fs:SetJustifyH("CENTER"); fs:SetTextColor(DIM[1], DIM[2], DIM[3], DIM[4])
  ...
  local progressHolder = CreateFrame("Frame", nil, frame); progressHolder:SetPoint("TOPLEFT", frame.previous, "TOPRIGHT", 6, 0); progressHolder:SetPoint("BOTTOMRIGHT", frame.continue, "BOTTOMLEFT", -6, 0)
  local progressBg = Solid(progressHolder, "BACKGROUND", .075, .113, .141, .9); progressBg:SetAllPoints(); Border(progressHolder, .25)
  frame.progress = CreateFrame("StatusBar", nil, progressHolder); frame.progress:SetPoint("TOPLEFT", 1, -1); frame.progress:SetPoint("BOTTOMRIGHT", -1, 1); frame.progress:SetStatusBarTexture(TEX); frame.progress:SetStatusBarColor(ACCENT[1], ACCENT[2], ACCENT[3], .9)
...
RUI._naowhBeta20InstallerLoaded = true
RUI._naowhBeta20InstallerRevision = 20
