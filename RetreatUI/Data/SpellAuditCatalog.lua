...
local function ParseSparseExtras(value)
  local result = {relations={}, relatedSpellIDs={}}
  if type(value) ~= "string" or value == "" then return result end
  for token in string.gmatch(value, "[^;]+") do
    ...
    elseif prefix == "e" then
      ...
      if id then
        result.effectID = id
        result.auraID = id
        result.effectKind = EFFECT_KIND_CODES[kind] or "aura"
        result.effectConfidence = "high"
      end
    elseif prefix == "l" then
      result.relations, result.relatedSpellIDs = ParseRelations(body)
    end
  end
  return result
end
...
function RUI:RegisterRawAuditSpellCatalog(className, raw)
  ...
  self.auditSpellCatalogByID[className] = nil
  return true
end
...
function RUI:RegisterCompactAuditSpellCatalog(className, specs, raw)
  ...
  self.auditSpellCatalogCompact[className] = {specs=specs, chunks={raw}}
  self.auditSpellCatalogCache[className] = nil
  self.auditSpellCatalogByID[className] = nil
  return true
end
...
function RUI:RegisterCompactAuditSpellCatalogChunk(className, specs, raw)
  ...
  if type(container) ~= "table" then
    container = {specs=specs, chunks={}}
    self.auditSpellCatalogCompact[className] = container
  end
  if type(container.specs) ~= "table" or #container.specs == 0 then container.specs = specs end
  container.chunks = container.chunks or {}
  container.chunks[#container.chunks + 1] = raw
  self.auditSpellCatalogCache[className] = nil
  self.auditSpellCatalogByID[className] = nil
  return true
end
...
local function ParseCompact(container)
  ...
  local chunks = container.chunks or (container.raw and {container.raw}) or {}
    ...
    for line in string.gmatch(raw or "", "[^\r\n]+") do
      ...
      if id then
        ...
        if f[5] and string.find(f[5], "[cdrhsel]", 1) then
          local extras = ParseSparseExtras(f[5])
          for key, value in pairs(extras) do record[key] = value end
        else
...
function RUI:GetAuditSpellCatalog(className)
  ...
  for _, record in ipairs(result) do
    if tonumber(record.id) and byID[tonumber(record.id)] == nil then byID[tonumber(record.id)] = record end
  end
  self.auditSpellCatalogByID[className] = byID
