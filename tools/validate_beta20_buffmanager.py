...
manifest = json.loads((ROOT / ".github/release-manifest.json").read_text(encoding="utf-8"))

expected_version = str(manifest["version"])
for name, toc in (("core", core_toc), ("classes", classes_toc), ("buff", buff_toc)):
    assert f"## Version: {expected_version}" in toc, f"{name} version mismatch: expected {expected_version}"
...
print(f"Buff Manager validation passed: separate optional package and synchronized version {expected_version}")
