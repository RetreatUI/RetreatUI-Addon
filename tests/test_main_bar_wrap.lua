...
local input={}
for i=1,11 do input[i]={name="Offensive "..i} end
local first, overflow=RUI:SplitMainBarDefinitions(input,9)
assert(#first==9 and #overflow==2,"split must be 9 + rest")
assert(first[9].name=="Offensive 9" and overflow[1].name=="Offensive 10","split order changed")
...
assert(firstY==0 and ninthY==0,"first nine offensive icons must stay on Main line 1")
assert(tenthY==-39 and eleventhY==-39,"remaining offensive icons must stay on Main line 2")
assert(core.__ruiMainBarFirstLineCount==9,"Main line 1 must contain nine")
assert(core.__ruiMainBarOverflowCount==2,"Main line 2 must contain the remaining offensives")

assert(utility.icons[1].definition.name=="Wall","defensive must stay first in Utility input")
assert(utility.icons[2].definition.name=="Kick","utility entries must remain on Utility")
assert(utility.icons[3].definition.name=="Dash","utility entries must remain on Utility")
assert(not utility.icons[4],"offensive overflow must never be merged into Utility")

local _,_,_,_,utilityY=utility:GetPoint()
assert(utilityY==-263,"Utility must move below wrapped Main")
...
assert(restoredY==-224,"Utility must return when Main no longer wraps")

print("RetreatUI offensive Main wrap and defensive Utility tests passed")
